...
#we store these tokens as every user has their own token (well,host of each room)
...
def update_or_create_user_tokens(session_key,access_token,token_type,expires_in,refresh_token):
    ...
    #spotify gives "expires in" in seconds. we want to store the actual time
    expires_in = timezone.now() + timedelta(seconds=expires_in)
    if tokens:
        #we update our tokens in the database
...
def execute_spotify_api_request(session_key, endpoint,post_=False,put_=False):
    #we do post_=False etc because this function can handle many different requests.
    #use underscores because there are already in built functions called put and post that we'll use
    tokens = get_user_tokens(session_key)
    headers = {
